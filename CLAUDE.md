...
- Меню (`features/lib/menu/`, 2026-09-12): живая куча фруктов под интерфейсом — `MenuFruitPile` → `MenuPileGame extends SplashGame with TapCallbacks` (`engine/menu_pile_game.dart`; 14 фруктов на ширину телефона, не крупнее t7, интервал 45 мс, тап по фрукту — `poke`: подскок, вращение, «ойк»). Контент — колонка до 420 pt по центру, лого 64 на экранах шире 600 pt (одной строкой через `FittedBox`), иконки звука и настроек под кнопкой «Играть», низ экрана отдан куче. Статичный коллаж пробовали и отклонили.
