...
def sfx():
    ...
    # Shake bonus: rattle — a burst of short soft knocks with falling pitch.
    buf = silence(0.55)
    rnd = random.Random(7)
    for i in range(9):
        t0 = i * 0.055 + rnd.uniform(0, 0.012)
        f = rnd.choice((260, 330, 410, 520))
        add(buf, [s * 0.55 for s in note(f, 0.07, decay=8, freq_end=f * 0.6)], t0)
        add(buf, [s * 0.3 for s in noise_burst(0.02, decay=10, seed=20 + i)], t0)
    write_wav('sfx_shake.wav', buf)

    # Bomb bonus: soft cartoon boom — low sweep + noise thump.
    buf = silence(0.6)
    add(buf, note(140, 0.5, decay=5, attack=0.003, freq_end=40, harmonics=((1, 1.0), (2, 0.3))), 0)
    add(buf, [s * 0.6 for s in noise_burst(0.25, decay=9, seed=3)], 0)
    add(buf, [s * 0.35 for s in noise_burst(0.05, decay=6, seed=4)], 0)
    write_wav('sfx_bomb.wav', buf)
