...
## Текущий статус: **1.0.0 отклонена App Review (4.3(b) Spam, 2026-09-17) → готовим 1.1 с отличиями от клонов**

План 1.1: [2026-09-17_release_1.1_after_4.3b.md](2026-09-17_release_1.1_after_4.3b.md) — Game Center, режимы «На время» и «Ежедневный вызов», онбординг, полировка, новый листинг без слов «suika/watermelon game», TestFlight с тестерами, отправка не раньше чем через 3–4 недели. Ответ в Resolution Center: `my_docs/review_reply_1.0.0_4.3b.txt`. Монетизация — рекомендовано отложить в 1.2 (ждёт решения).
...
- [x] «Оценить приложение» в настройках: `ReviewService` (`in_app_review`, `openStoreListing` по `AppConstants.appStoreId`), системный `requestReview` — метод есть, триггер не подключён (2026-09-14)
...
### Фаза 4: Релиз
Решение 2026-09-11: 1.0 выходит **только на iOS** (Android — позже). Код покупки и рекламы готов, но **2026-09-12 решено выпускать 1.0 без монетизации** (`AppConfig.monetizationEnabled` выключен): рекламные сети требуют опубликованное приложение (письмо AppLovin от 2026-09-11). Монетизация включается флагом `--dart-define=monetization=on` в `script/build.sh` — **в 1.2** (решение 2026-09-17: 1.1 идёт на ревью после 4.3(b) без рекламы и покупки, чтобы не расширять поверхность ревью).
- [x] **Покупка «Премиум навсегда»** (2026-09-11) — non-consumable `com.wasdrop.premium.lifetime`, 7.99 USD, локализации на 6 языков в ASC; Agreements/Tax/Banking приняты, sandbox-тестер есть. В коде: `PremiumService` (`in_app_purchase`, StoreKit 2, кэш в Hive), paywall `/premium`, механизм замков на темах (пока все бесплатны, 2026-09-11), «Восстановить покупки», секция «Премиум» в настройках. Тест: схема dev из Xcode + `ios/Runner/Products.storekit`; sandbox — prod на устройстве.
...
- [ ] **Перед выкладкой 1.0 (без монетизации)**: не прикреплять покупку к версии в ASC; App Privacy — Data Not Collected (SDK рекламы слинкован, но не инициализируется). `SKAdNetworkItems` и `NSUserTrackingUsageDescription` из Info.plist убраны (2026-09-12); `GADApplicationIdentifier` остаётся — без него слинкованный SDK роняет приложение на старте.
- [ ] **Перед 1.2 (монетизация; перенесено с 1.1 2026-09-17)**: вернуть в Info.plist `SKAdNetworkItems` (50 id) и `NSUserTrackingUsageDescription` (из git, коммит до «Ship 1.0 without monetization»); в `script/build.sh` добавить `--dart-define=monetization=on`; (1) аккаунт AdMob → App ID и два rewarded-блока в `AdsConfig` и `GAD_APPLICATION_ID` (pbxproj, prod-конфигурации); (2) ~~URL политики~~ — есть, `AppConstants.privacyPolicyUrl`; (3) App Privacy в ASC (см. `STORE_BRIEF.md`); (4) скриншот paywall и заметка в карточке покупки; (5) ~~в AdMob — опубликовать GDPR- и IDFA-сообщения (Privacy & messaging), добавить test device~~ — сделано 2026-09-14; (6) прогнать sandbox-покупку и restore на prod-сборке, ролики на dev.
- [ ] Ключ подписи Android (`android/key.properties`), Apple Team / профили
- [ ] Store-листинги (`.claude/my_docs/STORE_LISTINGS.md`), скриншоты
- [x] Первый релиз 1.0.0 (iOS, без монетизации) — отправлен на ревью 2026-09-14, **отклонён 2026-09-17 по 4.3(b) Design — Spam**, после ответа в Resolution Center добавлен **4.1(a) Copycats** (метаданные и иконка отсылают к Suika Game: слово suika в описании/keywords/скриншотах, арбуз с лицом на иконке → новая иконка без арбуза `my_docs/TZ_APP_ICON_V3.md`, метаданные из `my_docs/STORE_LISTINGS.md`, в японском スイカ заменить на «самый большой фрукт») (шаблонный отказ по жанру suika; аккаунт чистый, отказ первый). Дальше — план 1.1 (см. статус вверху); процесс: [my_docs/RELEASE_PROCESS.md](../my_docs/RELEASE_PROCESS.md)
- [ ] **AdMob настроен (2026-09-14)**: приложение iOS, блоки Continue/Refill (ID в `AdsConfig`/pbxproj), тестовое устройство, сообщения GDPR / США / IDFA опубликованы. После публикации: «Добавить магазин» (Apple ID 6809236566), `app-ads.txt`

### Фаза 5: Сервисы (после 1.0 — нужен аккаунт и сеть)
- [x] **Лидерборд iOS** — сделано в 1.1 (2026-09-17, `GameCenterService`, `my_docs/GAME_CENTER_ASC.md`); Android (Play Games) — ниже по старому описанию. Было: `games_services` (Game Center на iOS, Play Games Services на Android). Что нужно: App Store Connect → приложение → Game Center включить, завести Leaderboard (ID, формат «очки, больше — лучше»); Xcode → capability Game Center (entitlement). Play Console → Play Games Services → создать игровой проект, OAuth-клиент с SHA-1 ключа подписи (release + debug), Leaderboard ID, `android/app/src/main/res/values/games-ids.xml` + `APP_ID` в манифесте; приложение в Play должно существовать. В коде: вход (тихий при старте, кнопка в меню), `submitScore` при проигрыше и при новом рекорде, кнопка «Рекорды» в меню и на экране проигрыша (иконка кубка есть). Без сети — просто не показываем.
...
## Бэклог / идеи
- Мягкий системный запрос оценки (`ReviewService.requestReview`, StoreKit ≤ 3 показа/год): триггер вроде «третья партия выше рекорда», один раз, флаг «уже просили» в настройках — включать вместе с монетизацией
...
| 2026-09-17 | [Релиз 1.1 после отказа 4.3(b): заказы, стаканы, режимы, Game Center](2026-09-17_release_1.1_after_4.3b.md) → [план реализации](2026-09-17_impl_jars_missions_modes.md) | 🔨 В работе: этапы A–E готовы 2026-09-17 (стаканы, заказы, режимы, Game Center + ASC, онбординг и полировка); дальше F — TestFlight, листинг, отправка |
