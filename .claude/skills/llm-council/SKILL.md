...
argument-hint: "[question or decision to council]"
allowed-tools: Agent, Glob, Read, Write, Bash
---
...
This is adapted from Andrej Karpathy's LLM Council. He dispatches queries to multiple models, has them peer-review each other anonymously, then a chairman produces the final answer. We do the same thing inside Claude using sub-agents (the `Agent` tool) with different thinking lenses instead of different models.
...
- Recent council transcripts in `.claude/council/` (to avoid re-counciling the same ground)
...
Spawn all 5 advisors simultaneously as sub-agents via the `Agent` tool (one message, 5 parallel `Agent` calls). Each gets:
...
### step 5: present the verdict and save the transcript

The output is markdown only — no HTML, no browser. Two things:

**A. Show the chairman's verdict directly in the chat.** This is what the user reads. Use the chairman's exact 5-section structure (Where the Council Agrees / Clashes / Blind Spots / The Recommendation / The One Thing to Do First). Keep it scannable.

**B. Save the full transcript** to `.claude/council/council-[timestamp].md` with a single `Write` call. Get the timestamp first with a `Bash` call: `date +%Y%m%d-%H%M%S` (this is the ONLY use of `Bash`; `Write` creates the `.claude/council/` directory automatically). The transcript includes:
...
This transcript is the artifact. If the user wants to run the council again on the same question after making changes, the previous transcript lets them (or a future agent) see how the thinking evolved. After saving, just tell the user the path — don't dump the file contents back into the chat.

---

## output format

The user reads the verdict inline in the chat. One file is saved per session:

```
.claude/council/council-[timestamp].md   # full transcript for reference
```

Nothing else is written to disk, and no browser is opened.
...
- **Output is markdown only.** Present the verdict inline in the chat and save one transcript file. Don't generate HTML, don't open a browser, don't write extra files.
- **Stay in your lane.** The skill only reads context, spawns advisor sub-agents, and writes the transcript. It must not edit project code, touch git, or run anything beyond the single `date` call.
