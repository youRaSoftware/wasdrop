...
Overlay scrim: `Color(0x802B210E)` (pause) / `Color(0x8C2B210E)` (game over) — literal today; add `AppColors.scrim` if a third overlay appears.

**Rule:** never write `Color(0xFF…)` / `Colors.x` in `features/` — add a constant to `AppColors` and use it. Exceptions that exist today (overlay scrims, `Colors.white` in `AppFonts.button`) are known debt.
...
|---|---|---|
| `worldWidth` | 360 | Physics world width; jar aspect 310:440 → height `worldWidth * 440 / 310` |
| `ballRadii` | 6.7 … 62.0 | Radius per tier (world units), index = `tier.index` |
...
| `deadlineTopOffset` | 38 | Game-over line offset from jar top (world units) |
...
| `PrimaryButton` | `primary_button.dart` | `label`, `onPressed`, `height = 56` — `accentTop → accent` gradient, `accentShadow` offset `(0, 5)`, radius `height / 2`, `AppFonts.button` |
| `IconCircleButton` | `icon_circle_button.dart` | `child`, `onPressed` — 52 px circle, `surface` fill, 2 px `stroke` border |
| `BallView` | `ball_view.dart` | `tier`, `diameter`, `showEmoji = true` — static ball for HUD / menu (in-game balls are drawn by the engine) |

Feature-local widgets today: `features/lib/game/widgets/` → `GameHud`, `PauseOverlay` (+ private `_SecondaryButton`), `GameOverOverlay`. `_SecondaryButton` (52 px, `secondarySurface` fill, `secondaryText` label) is the first candidate for promotion to `core_ui` as `SecondaryButton` when a second consumer appears.

Missing but expected by the spec — create via `create-widget` when needed: `SecondaryButton`, `AppToggle` (sound / haptics switches), `OverlayPanel` (288 px `surface` card, radius 24, padding 24 — currently duplicated in both overlays), `RecordBadge` (gold gradient chip).
...
`MenuScreen` is the exception (a `StatefulWidget` calling the repository directly) — treat it as debt, not a pattern; new screens get a cubit.
...
`features/lib/game/engine/wasdrop_game.dart` (`WasDropGame extends Forge2DGame`) owns physics, input (drag = aim, release / tap = drop) and rendering of balls (`BallBody`). It talks to the cubit through explicit calls only: `cubit.onDropped()`, `cubit.onMerge(tier)`, `cubit.gameOver()`, and reads `cubit.state.current` for the next ball. The Form sets `_game.paused` from `state.status` and calls `_game.reset()` on restart. Keep UI (overlays, HUD) in Flutter widgets, not in Flame components.
...
### 4.8 Static analysis + formatting
